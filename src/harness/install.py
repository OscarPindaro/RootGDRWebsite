...
SERVER_NAME = "fastapi-template-test-harness"
...
def _interactive(
    ...
    detected = sorted(TARGETS, key=lambda target: not is_detected(target))
    for index, target in enumerate(detected):
        marker = "(detected)" if is_detected(target) else ""
        check = "[x]" if default_checked and is_detected(target) else "[ ]"
        print(f"  {index + 1}. {check} {target.display_name} {marker}".strip())
    ...
    if raw.lower() == "a":
        ...
        chosen = [target for target in detected if is_detected(target)]
    else:
        try:
            indices = [int(value) - 1 for value in raw.split()]
            chosen = [
                detected[index] for index in indices if 0 <= index < len(detected)
            ]
        except ValueError:
            print("  Invalid input.")
            return
    ...
    for target in chosen:
        print(f"    → {target.display_name} ({target.path})")
